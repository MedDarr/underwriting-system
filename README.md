# underwriting-system

Программная система поддержки андеррайтинга страхования частных домов физических лиц.

## Назначение проекта

Проект предназначен для автоматизации обработки заявок на страхование частных домов физических лиц, формирования электронного досье, выполнения проверок, оценки факторов риска, поддержки принятия андеррайтерского решения и формирования отчетности.

## Основные модули

- регистрация и обработка заявок;
- электронное досье и документы;
- правила андеррайтинга и оценка риска;
- проверки безопасности и страховой истории;
- поддержка принятия решения;
- отчетность и аналитика;
- пользователи, роли и разграничение доступа;
- аудит и журналирование действий.

## Технологический стек

- Frontend: React, TypeScript;
- Backend: Node.js, NestJS, TypeScript;
- Database: PostgreSQL;
- ORM: Prisma;
- Version control: Git, GitHub.

## Структура репозитория

```text
underwriting-system/
├── backend/                  # серверная часть
├── frontend/                 # клиентская часть
├── docs/                     # документация и схемы
├── prisma/                   # схема данных и миграции
├── tests/                    # тестовые материалы
├── .gitignore
├── package.json
└── README.md
```

## Статус

Инициализирующий коммит лабораторной работы №3.
